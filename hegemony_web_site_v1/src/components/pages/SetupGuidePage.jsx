...
import { CheckCircle2, Circle, Package, Users } from 'lucide-react';
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="bg-surface-primary border border-border rounded-xl p-6">
                ...
                <div className="flex justify-center gap-2 md:gap-4 mb-6 md:mb-8 w-full">
                    {[2, 3, 4].map((count) => (
                        <button
                            key={count}
                            onClick={() => setNumPlayers(count)}
                            className={`flex flex-col items-center justify-center flex-1 py-3 md:py-4 rounded-xl border-2 transition-all duration-300 ${numPlayers === count
                                ? 'bg-blue-600 border-blue-400 text-white shadow-lg scale-105'
                                : 'bg-surface-secondary border-border text-muted-foreground hover:bg-surface-tertiary hover:border-border'
                                }`}
                        >
                            <span className="text-xl md:text-3xl font-bold mb-1">{count}</span>
                            <span className="text-[10px] md:text-xs font-bold uppercase tracking-wider">Joueurs</span>
                        </button>
                    ))}
                </div>

                {/* Active Classes Display */}
                <div className="mt-6 pt-4 border-t border-border flex justify-center gap-4 md:gap-6 overflow-x-auto pb-2">
                    <div className="flex items-center justify-center p-2 bg-surface-secondary rounded-xl min-w-[50px]">
                        <ClassReference type="working" showLabel={false} size="h-8 w-8 md:h-10 md:w-10" />
                    </div>
                    <div className="flex items-center justify-center p-2 bg-surface-secondary rounded-xl min-w-[50px]">
                        <ClassReference type="capitalist" showLabel={false} size="h-8 w-8 md:h-10 md:w-10" />
                    </div>
                    <div className={`flex items-center justify-center p-2 rounded-xl transition-all duration-300 min-w-[50px] ${numPlayers >= 3 ? 'bg-surface-secondary opacity-100' : 'bg-muted opacity-30 grayscale'}`}>
                        <ClassReference type="middle" showLabel={false} size="h-8 w-8 md:h-10 md:w-10" />
                    </div>
                    <div className={`flex items-center justify-center p-2 rounded-xl transition-all duration-300 min-w-[50px] ${numPlayers === 4 ? 'bg-surface-secondary opacity-100' : 'bg-muted opacity-30 grayscale'}`}>
                        <ClassReference type="state" showLabel={false} size="h-8 w-8 md:h-10 md:w-10" />
                    </div>
                </div>
            </div>
            ...
            <Card className="bg-surface-primary border-2 border-green-800">
